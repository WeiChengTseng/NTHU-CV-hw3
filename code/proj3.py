...
def main():
    ...
    if FEATURE == 'tiny_image':
        ...
        if os.path.isfile('vocab{}.pkl'.format(vocab_size)) is False:
            print('No existing visual word vocabulary found. Computing one from training images\n')
            
            vocab = build_vocabulary(train_image_paths, vocab_size)
            with open('vocab{}.pkl'.format(vocab_size), 'wb') as handle:
                pickle.dump(vocab, handle, protocol=pickle.HIGHEST_PROTOCOL)

        if os.path.isfile('train_image_feats{}.pkl'.format(vocab_size)) is False:
            # YOU CODE get_bags_of_sifts.py
            train_image_feats = get_bags_of_sifts(train_image_paths, vocab_size);
            with open('train_image_feats{}.pkl'.format(vocab_size), 'wb') as handle:
                pickle.dump(train_image_feats, handle, protocol=pickle.HIGHEST_PROTOCOL)
        else:
            with open('train_image_feats{}.pkl'.format(vocab_size), 'rb') as handle:
                train_image_feats = pickle.load(handle)

        if os.path.isfile('test_image_feats{}.pkl'.format(vocab_size)) is False:
            test_image_feats  = get_bags_of_sifts(test_image_paths, vocab_size);
            with open('test_image_feats{}.pkl'.format(vocab_size), 'wb') as handle:
                pickle.dump(test_image_feats, handle, protocol=pickle.HIGHEST_PROTOCOL)
        else:
            with open('test_image_feats{}.pkl'.format(vocab_size), 'rb') as handle:
    ...
    if CLASSIFIER == 'nearest_neighbor':
        ...
        predicted_categories = svm_classify(train_image_feats, train_labels, test_image_feats, 'rbf')
...
def plot_confusion_matrix(cm, category, title='Confusion matrix', cmap=plt.cm.Blues):
    ...
    plt.savefig('../results/bag_svm_rbf.png')
